...
func TestValidAgentMetaKey(t *testing.T) {
	for key, want := range map[string]bool{
		"model": true, "context_used": true, "cost-usd": true, "a1": true,
		"": false, "Model": false, "1st": false, "_x": false, "with space": false,
		"$model": false, strings.Repeat("a", AgentMetaMaxKey): true,
		strings.Repeat("a", AgentMetaMaxKey+1): false,
	} {
		if got := ValidAgentMetaKey(key); got != want {
			t.Errorf("ValidAgentMetaKey(%q) = %v, want %v", key, got, want)
		}
	}
}
...
// TestSetAgentMetaVerb walks the verb end to end: set, read back through
// get-agent-state and list-agents, refuse a bad key, and survive a client push
// that does not carry the field.
func TestSetAgentMetaVerb(t *testing.T) {
	d, sp := startTestDaemon(t)
	sess := makeSessionWithWindow(t, d, "work")
	c := dialVerb(t, sp)

	res := result(t, c.call(t, `{"id":1,"verb":"set-agent-meta","params":{"session":"work","window":"Window","source":"statusline","tokens":{"model":"opus","context":"42%"}}}`))
	meta, _ := res["meta"].(map[string]any)
	if meta["model"] != "opus" || meta["context"] != "42%" {
		t.Fatalf("set-agent-meta result meta = %v", res["meta"])
	}
	id := windowIDOf(t, sess)
	w := sess.GetState().Windows[0]
	if w.ID != id || !slices.Equal(metaKeys(w.AgentMeta), []string{"model=opus", "context=42%"}) {
		t.Fatalf("state holds %v", metaKeys(w.AgentMeta))
	}

	got := result(t, c.call(t, `{"id":2,"verb":"get-agent-state","params":{"session":"work","window":"Window"}}`))
	if m, _ := got["meta"].(map[string]any); m["model"] != "opus" {
		t.Errorf("get-agent-state meta = %v", got["meta"])
	}
	list := result(t, c.call(t, `{"id":3,"verb":"list-agents","params":{"session":"work","all":true}}`))
	agents, _ := list["agents"].([]any)
	if len(agents) != 1 {
		t.Fatalf("list-agents = %v", list)
	}
	if m, _ := agents[0].(map[string]any)["meta"].(map[string]any); m["context"] != "42%" {
		t.Errorf("list-agents meta = %v", agents[0])
	}

	if code := errCode(t, c.call(t, `{"id":4,"verb":"set-agent-meta","params":{"session":"work","window":"Window","tokens":{"Bad Key":"x"}}}`)); code != ErrVerbInvalidParams {
		t.Errorf("a bad key gave %q, want invalid_params", code)
	}
	if code := errCode(t, c.call(t, `{"id":5,"verb":"set-agent-meta","params":{"session":"work","window":"Window"}}`)); code != ErrVerbInvalidParams {
		t.Errorf("no tokens and no clear gave %q, want invalid_params", code)
	}

	// A client push never carries metadata, and must not wipe it.
	push := sess.GetState()
	for i := range push.Windows {
		push.Windows[i].AgentMeta = nil
	}
	retainDaemonExclusive(push, sess.GetState())
	if len(push.Windows[0].AgentMeta) != 2 {
		t.Errorf("a client push without metadata wiped it: %v", push.Windows[0].AgentMeta)
	}
}
