...
func TestAttentionListOrderAndFilters(t *testing.T) {
	a, _ := recordingAttention()
	a.noteSessionEvent("s", agentEvent("f", "working", "idle", "", "", 0, 1))
	time.Sleep(time.Millisecond)
	a.noteSessionEvent("s", agentEvent("q2", "working", "needs_input", "question", "later", 0, 0))
	a.noteSessionEvent("t", agentEvent("e", "working", "errored", "", "", 0, 0))
	a.noteSessionEvent("s", agentEvent("ap", "working", "needs_input", "approval", "", 0, 0))
	a.noteMail(AgentMessage{Kind: agentMsgDirect, Session: "s", From: "m", FromLabel: "planner", To: AgentInboxHuman, Subject: "done?", ThreadID: 7})
	...
	if got, want := strings.Join(kinds, ","), "approval,question,mail,errored,finished"; got != want {
		t.Errorf("order %s, want %s", got, want)
	}

	sel, counts, _ := a.list(attentionQuery{session: "s", kinds: map[string]bool{AttentionQuestion: true, AttentionFinished: true}})
	if len(sel) != 2 {
		t.Errorf("filtered listing has %d items, want 2", len(sel))
	}
	if counts[AttentionErrored] != 1 || counts[AttentionApproval] != 1 {
		t.Errorf("counts %v are not over the whole queue", counts)
	}
}
