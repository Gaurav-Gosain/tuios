...
// TestAHostChangedEventReplacesThePoll is what the client listens for: a
// change on a host is pushed as host-changed.
func TestAHostChangedEventReplacesThePoll(t *testing.T) {
	hub, far := startHubAndFar(t)
	sess := makeSessionWithWindow(t, far.daemon, "remote-work")
	waitForHostUp(t, hub, "build")
	waitFleetLive(t, hub, "build")
	...
	sub := hubVerb(t)
	result(t, sub.call(t, `{"id":1,"verb":"subscribe","params":{"types":["host-changed"]}}`))
	farC := dialVerb(t, far.socket)
	setAgentState(t, farC, "remote-work", sess.GetState().Windows[0].ID, "working", "", "")

	ev := readStreamEvent(t, sub)
	if ev["type"] != EventHostChanged || ev["host"] != "build" || ev["status"] != "up" {
		t.Fatalf("the push is %v", ev)
	}
}
