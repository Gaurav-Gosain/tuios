...
func TestFanMixesAgentsFromTheCallersPath(t *testing.T) {
	...
	for _, bad := range []map[string]any{
		{"agent": "claude", "agents": []string{"codex"}, "prompt": "x", "count": 1},
		{"agent": "claude", "prompt": "x", "prompts": []string{"y"}},
		{"agent": "claude", "prompts": []string{"a", "b"}, "count": 3},
		{"agent": "claude", "prompts": []string{"a", " "}},
		{"agent": "./relative/tool", "prompt": "x", "count": 1},
	} {
		bad["repo"] = repo
		if code := errCode(t, callVerb(t, c, "fan", bad)); code != ErrVerbInvalidParams {
			t.Errorf("fan %v: code %q, want %q", bad, code, ErrVerbInvalidParams)
		}
	}
}
