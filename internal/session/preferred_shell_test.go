...
// A shell the client named in its hello still wins over the daemon's setting.
func TestHelloShellWinsOverThePreferredShell(t *testing.T) {
	dir := t.TempDir()
	preferred := linkShell(t, dir, "preferred-sh")
	named := linkShell(t, dir, "named-sh")

	m := NewManager()
	m.SetPreferredShell(preferred)
	sess, err := m.CreateSession("hello-shell", &SessionConfig{Shell: named}, 80, 24)
	if err != nil {
		t.Fatalf("CreateSession: %v", err)
	}
	defer sess.Stop()
	if got := sess.getShell(); got != named {
		t.Errorf("getShell = %q, want the hello's shell %q", got, named)
	}
}
