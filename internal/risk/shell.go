...
// command substitutions and sh -c into the commands they run. It is a reader
// of what a person or a model types, not a shell: it knows quotes, escapes,
// the command separators, pipes and redirects, and nothing else.
func parseCommands(text string, depth int) []command {
	...
	for _, seg := range segments {
		...
		c := command{raw: strings.TrimSpace(seg.text), writes: writes}
		...
		out = append(out, c)
		// sh -c 'command' runs a command line of its own.
		if depth < maxDepth && isShellName(c.name()) {
			for i, a := range c.args() {
				if a == "-c" && i+1 < len(c.args()) {
					out = append(out, parseCommands(c.args()[i+1], depth+1)...)
					break
				}
			}
		}
...
// splitSegments splits text on ; & && || | |& and newlines outside quotes.
// The contents of $( ) and backticks are appended to nested, to be read as
...
func splitSegments(text string, nested *[]string) []segment {
	...
	flush := func(nextPiped bool) {
		out = append(out, segment{text: cur.String(), piped: piped})
		cur.Reset()
		piped = nextPiped
	}
