...
func TestOpenCodePluginRoundTrip(t *testing.T) {
	...
	if !strings.Contains(plugin, `const TUIOS = "/opt/my tuios/tuios";`) || !strings.Contains(plugin, "TUIOS_INTEGRATION_VERSION=1") {
		t.Fatalf("plugin was not filled in:\n%s", plugin[:400])
	}
	if strings.Contains(plugin, "__TUIOS_") {
		t.Fatal("a placeholder was left in the plugin")
	}
	if st := tg.Status(env, "/opt/my tuios/tuios"); !st.Installed || !st.Current || st.Version != 1 {
