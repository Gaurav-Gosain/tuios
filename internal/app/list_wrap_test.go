...
// TestHelpSectionsWrap is the tab rule on the help overlay's section strip.
func TestHelpSectionsWrap(t *testing.T) {
	m := wrapOS()
	m.KeybindRegistry = config.NewKeybindRegistry(m.UserConfig)
	n := len(m.HelpCategories())
	m.HelpCategory = 0
	m.HelpStepCategory(-1)
	if m.HelpCategory != n-1 {
		t.Errorf("left on the first help section went to %d, want %d", m.HelpCategory, n-1)
	}
	m.HelpStepCategory(1)
	if m.HelpCategory != 0 {
		t.Errorf("right on the last help section went to %d, want 0", m.HelpCategory)
	}
}
