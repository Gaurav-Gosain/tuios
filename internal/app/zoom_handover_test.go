...
// TestAFocusAppliedBySyncLeavesTheZoom pins that a peer's focus, which arrives
// by assignment rather than through FocusWindow, does not drag this client's
// zoom around.
func TestAFocusAppliedBySyncLeavesTheZoom(t *testing.T) {
	m, wins := zoomPeekOS(t)
	m.Settings.ZoomAnimation = false
	m.AutoTiling = false
	m.FocusedWindow = 0
	...
	if !wins[0].Zoomed || wins[1].Zoomed {
		t.Errorf("a focus written by sync moved the zoom: [0].Zoomed=%t [1].Zoomed=%t",
			wins[0].Zoomed, wins[1].Zoomed)
	}
}
