...
func sidebarStripControlCell(glyph string, cw int, edgeLeft, lit bool, bg color.Color, pal overlay.Palette) string {
	// At raw FgMute this measured 2.19:1 against the band: the strip's way out
	// was one of its least visible marks. Lifted to 4.88:1 for the same reason
	// the tail mark is, and by the same call.
	fg := theme.Readable(pal.FgMute, bg)
	if lit {
		fg = pal.Fg
	}
