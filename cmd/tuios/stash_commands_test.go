...
func TestStashPutSaysWhenItStoredNothing(t *testing.T) {
	...
	if !strings.Contains(notes.String(), "already stored") {
		t.Errorf("a duplicate put does not say so: %q", notes.String())
	}
}
...
func TestStashPutSaysWhenItDropped(t *testing.T) {
	...
	if !strings.Contains(notes.String(), "dropped 3 older file") {
		t.Errorf("the eviction is not reported: %q", notes.String())
	}
}
