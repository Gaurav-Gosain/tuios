...
| A machine's group does not fold | n/a, injected, cuts the wiring | invert the empty-name guard in `SidebarToggleHostCollapsed` so every call returns early | `TestRailAttachesARemoteSessionInThisClient` ("the click on the header did not fold the group"); unit `TestHostHeaderClickFoldsTheGroup` | **caught** |
...
| A rail too narrow for a name and a number keeps the number on a machine's rows and drops it on this machine's, so the same row type answers the width two ways | n/a, injected | drop `variant == sidebarVariantFull` from the count in `sidebarRemoteSessionRow` in `internal/app/sidebar_hosts.go` | unit `TestNarrowRailDropsTheRemoteCount` ("a row on another machine keeps its count on a rail too narrow for one"), whose positive half is the same row carrying the count at the full variant | **caught in `internal/app`** |
| The width never reaches the row: `drawHostRow` takes the variant and hands the row a constant | n/a, injected, cuts the wiring | `drawHostRow` passes `sidebarVariantFull` to `sidebarRemoteSessionRow` instead of `variant` | unit `TestNarrowRailDropsTheRemoteCount` | **caught in `internal/app`**, and it is why that test drives `drawHostRow` rather than the row function |
...
`internal/app`. Both of those do fail with the reserve removed (four resizes
